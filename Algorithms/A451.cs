...
namespace Algorithms
{
    public class A451
    {
		...
		public string Input = "Tree";

		public void Run()
        {
			this.FrequencySort(Input);
        }
        ...
        private string FrequencySort(string s)
        {
            ...
            {
				if(sl.ContainsKey(s.ToCharArray()[i]))
                {
					sl[s.ToCharArray()[i]] = sl[s.ToCharArray()[i]]+1;
                }
                else
                {
					sl.Add(s.ToCharArray()[i], 1);
				}
            }

			Console.WriteLine("Input is : {0}", Input);
			Console.WriteLine("Frequency Sort Array : [{0}]", string.Join(", ", sl.OrderByDescending(x => x.Value).Select(y => y.Key)));
